...
public class SwiftFlutterCallkitIncomingPlugin: NSObject, FlutterPlugin, CXProviderDelegate {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "forceResetCallState":
            // KORTOBAA fork addition (2026-04-17, re-applied on 3.1.5): iOS
            // symmetric reset. Ends all active CallKit calls via
            // CXEndCallAction (the proper way on iOS) + defensively
            // deactivates the shared AVAudioSession so a rapid re-call
            // doesn't inherit stuck audio state. Parallel to Android's
    ...
    /// this via the `didDeactivateAudioSession` callback, but that callback
    /// fires asynchronously after `endCallAlls()` returns — and on a rapid
